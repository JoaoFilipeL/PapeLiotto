...
export function TodayOrdersTable() {
    ...
    const fetchTodayOrders = useCallback(async () => {
        ...
        try {
            ...
            const { data: ordersData, error: ordersError } = await supabase
                .from('orders')
                .select('id, order_code, customer_name, total_amount, status, delivery_time, customers!inner(phone)')
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('today_orders_realtime_v5')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, fetchTodayOrders)
        ...
        <div className="bg-[#2D2D2D] text-white border border-zinc-800 rounded-lg shadow-sm p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-2">
                    <h3 className="text-xl font-bold">Pedidos para Hoje</h3>
                ...
                {loading ? (
                    ...
                    filteredOrders.map((order) => (
                        <div key={order.id} className="grid grid-cols-1 md:grid-cols-12 items-center gap-x-4 bg-neutral-950/40 p-3 rounded-md">
                            ...
                            <div className="col-span-2 text-center">{order.delivery_time || 'N/A'}</div>
                        </div>
                    ))
                ) : (
                    <div className="text-center py-10 text-zinc-500">Nenhum pedido para hoje encontrado.</div>
                )}
            </div>
        </div>
    )
